...
class DecoratorFactory():
    ...
    @classmethod
    def calcualte_printer(self, func: F) -> F:
        '''
...
class OrderCost():
    ...
    def cost_id_df(self) -> pd.DataFrame:
        ...
        df = (df.merge(self._data.id_df(), left_on='shop_id', right_on='ID')).drop(columns="ID").rename({'Shop   Name  ':'shop_name'}, axis=1)
        # get all data consists of shop name Auna and Numan, excluding the country code
        df['shop_only_name'] = df['shop_name'].str.extract('(Auna|AUNA|Numan|NUMAN)')
        # making uppercase to all the shope name with lambda expression
        df.shop_only_name = df['shop_only_name'].apply(lambda x: x.upper())
...
class RevenueFactory():
    ...
    @DecoratorFactory.calcualte_printer
...
def main() -> None:
    ...
    # for already exisitng spread sheet we can use the spread sheed uniquie identifier or we can laso create a new spreadsheet with the api and create_spread_sheet function by giving any name
    #titel = "BBG_Report"
    #spreadSheetId = oc.gs_api.create_spread_sheet(service,creds,titel)
    ...
    # for already exisitng work sheet we can use the spread sheed name or we can laso create a new spreadsheet with the api and create_spread_sheet function
